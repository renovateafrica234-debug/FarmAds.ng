...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { data: order, error: oe } = await supabase.from("orders").select("*, produce(name, unit), farmers(full_name, phone, location)").eq("payaza_reference", p.merchant_reference).single();
    ...
    if (Math.abs(expected - received) > 1) { console.error(`Mismatch: expected ₦${expected}, got ₦${received}`); return res.status(200).json({ received: true }); }
    ...
    await supabase.from("orders").update({ status: "paid", paid_at: new Date().toISOString(), payaza_transaction_reference: p.transaction_reference }).eq("id", order.id);
...
async function sendWA(to: string, body: string) {
  try {
    ...
    await fetch(`https://gate.whapi.cloud/messages/text?token=${WHAPI_TOKEN}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ to: phone, body, typing_time: 0 }) });
