  ...
  try {
    ...
    if (lower === "menu" || text === "0" || lower === "start") {
      ...
      if (text === "1") { reply = catMenu(); nextStep = "browse_category"; nextCtx = {}; }
        ...
        else { reply = "📦 *Your Orders*\n\n"; orders.forEach((o: any, i: number) => { reply += `${i+1}. ${o.produce?.name||"Produce"} — ₦${o.total_amount?.toLocaleString()} (${o.status})\n`; }); reply += "\nType *MENU*."; }
      ...
      if (CATEGORIES[text]) {
        ...
        if (!produce?.length) { reply = "😔 No produce.\n\n" + catMenu(); nextStep = "browse_category"; }
        else {
      ...
      if (isNaN(idx) || idx < 0 || idx >= list.length) { reply = "❓ Invalid. Reply 1-" + list.length + "."; nextStep = "browse_produce"; }
      else {
        const p = list[idx];
        ...
        nextStep = "enter_quantity"; nextCtx = { ...ctx, selectedProduce: p, selectedProduceId: p.id, selectedFarmerId: p.farmer_id };
      }
      ...
      if (text === "1") {
        const { data: order, error: oe } = await supabase.from("orders").insert({ buyer_phone: phone, farmer_id: ctx.selectedFarmerId, produce_id: ctx.selectedProduceId, quantity_kg: qty, total_amount: total, delivery_location: loc, status: "pending" }).select().single();
